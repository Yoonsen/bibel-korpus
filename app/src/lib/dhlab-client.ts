...
export async function fetchDocumentCounts(
  ...
  const body = { urns: urns.length ? urns : null };
  const payload = await postJSON<Record<string, number> | null>(
    baseUrl,
    '/document_counts',
    body,
  );
...
export async function fetchAggregateCounts(
  baseUrl: string,
  urns: string[],
): Promise<number> {
  const counts = await fetchDocumentCounts(baseUrl, urns);
